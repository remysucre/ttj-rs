...
pub fn q22a(db: &ImdbData) -> Result<Option<(&str, &str, &str)>, PolarsError> {
    let cn = &db.cn;
    let ct = &db.ct;
    let it1 = &db.it;
    let it2 = &db.it;
    ...
    let t_m: HashMap<i32, Vec<&str>> = t
        ...
        .filter_map(|(((id, title), production_year), kind_id)| {
            ...
            {
                if production_year > 2008 && kt_s.contains(&kind_id) {
                    Some((id, title))
                } else {
                    None
                }
...
mod test_22a {
    ...
    #[test]
    fn test_q22a() -> Result<(), PolarsError> {
        let db = ImdbData::new();
        let res = q22a(&db)?;
        assert_eq!(res, Some(("01 Distribution", "2.1", "12 Rounds")));
        Ok(())
    }
}
