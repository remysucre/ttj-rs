...
pub fn q21b(db: &Data) -> Result<Option<(&str, &str, &str)>, PolarsError> {
    let cn = &db.cn;
    let ct = &db.ct;
    let k = &db.k;
    let lt = &db.lt;
    ...
    let target_info: HashSet<&str> = ["Germany", "German"].into_iter().collect();
    ...
    println!("21b,{:}", start.elapsed().as_secs_f32());
