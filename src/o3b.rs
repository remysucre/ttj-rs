...
pub fn q3b(db: &ImdbData) -> Result<Option<&str>, PolarsError> {
    let k = &db.k;
    let mi = &db.mi;
    let mk = &db.mk;
    let t = &db.t;
        ...
        .zip(t.column("title")?.str()?.into_iter())
    {
        ...
        {
            if production_year > 2010 && mk_s.contains(&movie_id) && mi_s.contains(&movie_id) {
...
mod test_3b {
    ...
    #[test]
    fn test_q3b() -> Result<(), PolarsError> {
        let db = ImdbData::new();
        let res = q3b(&db)?;
        assert_eq!(res, Some("300: Rise of an Empire"));
        Ok(())
    }
}
