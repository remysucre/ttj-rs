...
pub fn q31b(db: &ImdbData) -> Result<Option<(&str, &str, &str, &str)>, PolarsError> {
    let ci = &db.ci;
    let cn = &db.cn;
    let it1 = &db.it;
    let it2 = &db.it;
    ...
    let mc_s: HashSet<i32> = mc
        ...
        .filter_map(|((company_id, movie_id), note)| {
            if let (Some(company_id), Some(movie_id), Some(note)) = (company_id, movie_id, note) {
                if note.contains("(Blu-ray)") && cn_s.contains(&company_id) {
                    Some(movie_id)
                } else {
                    None
                }
        ...
        .zip(t.column("production_year")?.i32()?.into_iter())
    {
        if let (Some(id), Some(title), Some(production_year)) = (id, title, production_year) {
            if production_year > 2000
                && (title.contains("Freddy") || title.contains("Jason") || title.contains("Saw"))
