...
pub fn q29b(db: &ImdbData) -> Result<Option<(&str, &str, &str)>, PolarsError> {
    let an = &db.an;
    let cc = &db.cc;
    let cct1 = &db.cct;
    let cct2 = &db.cct;
    ...
    let mi_s: HashSet<i32> = mi
        ...
        .filter_map(|((movie_id, info), info_type_id)| {
            ...
            {
                if it_s.contains(&info_type_id) && info.starts_with("USA:") && info.contains("200")
                {
                    Some(movie_id)
                } else {
                    None
        ...
        .zip(t.column("title")?.str()?.into_iter())
    {
        if let (Some(id), Some(production_year), Some(title)) = (id, production_year, title) {
            if title == "Shrek 2" && (2000..=2005).contains(&production_year) {
    ...
    let it3_s: HashSet<i32> = it3
        ...
        .str()?
        .into_iter()
        .zip(it3.column("id")?.i32()?.into_iter())
        .filter_map(|(info, id)| {
            if let (Some(info), Some(id)) = (info, id) {
                if info == "height" { Some(id) } else { None }
            } else {
                None
            }
        })
                            ...
                            for title in titles {
                                for name in names {
                                    for chn_name in chn_names {
                                        if let Some((old_chn_name, old_name, old_title)) =
                                            res.as_mut()
                                        {
                                            if name < old_name {
                                                *old_name = name;
...
mod test_29b {
    ...
    #[test]
    fn test_q29b() -> Result<(), PolarsError> {
        let db = ImdbData::new();
        let res = q29b(&db)?;
