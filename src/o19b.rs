...
pub fn q19b(db: &ImdbData) -> Result<Option<(&str, &str)>, PolarsError> {
    let an = &db.an;
    let chn = &db.chn;
    let ci = &db.ci;
    let cn = &db.cn;
        ...
        .zip(n.column("gender")?.str()?.into_iter())
    {
        if let (Some(id), Some(name), Some(gender)) = (id, name, gender) {
            if an_s.contains(&id) && gender == "f" && name.contains("Angel") {
        ...
        .zip(mi.column("info")?.str()?.into_iter())
    {
        if let (Some(mid), Some(it_id), Some(info)) = (mid, it_id, info) {
            if ((info.starts_with("Japan:") && info.contains("2007"))
                || info.starts_with("USA:") && info.contains("2008"))
                && it_s.contains(&it_id)
            {
                mi_s.insert(mid);
            }
        ...
        .zip(t.column("production_year")?.i32()?.into_iter())
    {
        if let (Some(id), Some(title), Some(production_year)) = (id, title, production_year) {
            if title.contains("Kung")
                && title.contains("Fu")
                && title.contains("Panda")
