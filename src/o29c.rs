...
pub fn q29c(db: &ImdbData) -> Result<Option<(&str, &str, &str)>, PolarsError> {
    let an = &db.an;
    let cc = &db.cc;
    let cct1 = &db.cct;
    let cct2 = &db.cct;
    ...
    println!("29c,{:}", start.elapsed().as_secs_f32());
...
mod test_29c {
    ...
    #[test]
    fn test_q29c() -> Result<(), PolarsError> {
        let db = ImdbData::new();
        let res = q29c(&db)?;
        assert_eq!(res, Some(("Lola", "Andrews, Julie", "Hoodwinked!")));
        Ok(())
    }
}
